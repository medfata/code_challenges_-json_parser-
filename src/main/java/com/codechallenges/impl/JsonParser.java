...
import com.codechallenges.Constants;
import com.codechallenges.JsonParserException;
...
public class JsonParser {
    ...
    public static Object parse(String jsonString) {
        ...
        try {
            ...
            throw new JsonParserException("Provided JSON string did not contain a value");
        }
        ...
        if (current == '{') {
            ...
            throw new JsonParserException(stateStack, "Unexpected character \"" + current + "\" instead of root value");
        }
            ...
            switch (currentType) {
                case NAME:
                    try {
                        ...
                        throw new JsonParserException(stateStack, "String did not have ending quote");
                    }
                ...
                case NUMBER: {
                    ...
                    try {
                        ...
                        throw new JsonParserException(stateStack, "\"" + valueString +
                ...
                case HEURISTIC:
                    ...
                    if (current != ':' && expectingColon) {
                        stateStack.push(new State(propertyName, currentContainer, Type.OBJECT));
                        throw new JsonParserException(stateStack, "wasn't followed by a colon");
                    }
                    ...
                    if (current == ':') {
                        if (expectingColon) {
                            ...
                            stateStack.push(new State(propertyName, currentContainer, Type.OBJECT));
                            throw new JsonParserException(stateStack, "was followed by too many colons");
                        }
                    } else if (current == '"') {
                        currentType = Type.STRING;
                        fieldStart = i;
                        ...
                        throw new JsonParserException(stateStack, "unexpected character \"" + current +
                ...
                case OBJECT:
                    ...
                    if (current == ',') {
                        if (expectingComma) {
                            ...
                            stateStack.push(new State(propertyName, currentContainer, Type.OBJECT));
                            throw new JsonParserException(stateStack, "followed by too many commas");
                        }
                    } else if (current == '"') {
                        if (expectingComma) {
                            stateStack.push(new State(propertyName, currentContainer, Type.OBJECT));
                            throw new JsonParserException(stateStack, "wasn't followed by a comma");
                        }
                ...
                case ARRAY:
                    ...
                    if (current != ',' && current != ']' && current != '}' && expectingComma) {
                        ...
                        throw new JsonParserException(stateStack, "wasn't preceded by a comma");
                    }
                    ...
                    if (current == ',') {
                        if (expectingComma) {
                            ...
                            stateStack.push(new State(null, currentContainer, Type.ARRAY));
                            throw new JsonParserException(stateStack, "preceded by too many commas");
                        }
                    } else if (current == '"') {
                        currentType = Type.STRING;
                        fieldStart = i;
                        ...
                        stateStack.push(new State(propertyName, currentContainer, Type.ARRAY));
                        throw new JsonParserException(stateStack, "Unexpected character \"" + current + "\" instead of array value");
                    }
                    break;
            }
        }

        throw new JsonParserException("Root element wasn't terminated correctly (Missing ']' or '}'?)");
    }
